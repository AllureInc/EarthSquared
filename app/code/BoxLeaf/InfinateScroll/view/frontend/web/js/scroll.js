    ...
    function (Component, $, InfiniteScroll, mageTemplate, imagesloaded, bridget, Masonry, Isotope, Packery) {
        ...
        return Component.extend({
            defaults: {
                settings: {
                    enable: true,
                    $element: $('.column.main .product-items'),
                    analytics: true,
                    status: true,
                ...
                config: {
                    ...
                    append: '.column.main .item.product.product-item',
            ...
            initScroll: function () {
                ...
                self.settings.$element.on( 'append.infiniteScroll', function( event, response, path, items ) {
                        ...
                        var subcat,catlen;
                        $(".subcategory-name").each(function( index ) {
                          subcat = $(this).attr('data-id');
                          if(subcat !=''){
                            catlen = $('.subcategory-name.'+subcat).length;
                            if(catlen > 1){
                              var i= 0;
                              $(".subcategory-name."+subcat).each(function( index ) {
                                if(i > 0)
                                {
                                    $(this).hide();
                                }
