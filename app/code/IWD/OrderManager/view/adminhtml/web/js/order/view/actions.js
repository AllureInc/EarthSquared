    ...
    $.widget('mage.iwdOrderManagerActions', {
        ...
        _initUpdate: function(){
            ...
            $(document).on('click touchstart', this.options.updateButtonId, (function(e) {
                ...
                if(self.validateForm() && self._iwdBraintree()){
                    self.confirmUpdate();
                }
            }));
        },

        _iwdBraintree: function(){
            var payment_nonce = $('#braintree_payment_method_nonce').val(), result = true;
            $('#order-billing_method .admin__field-option input').each(function(){
                if($(this).attr('checked') == 'checked' && $(this).attr('id') == 'p_method_braintree' && !payment_nonce.length){
                    result = false;
                }
            });
            return result;
