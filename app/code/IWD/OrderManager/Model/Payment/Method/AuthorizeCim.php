...
class AuthorizeCim extends AbstractMethod
{
    ...
    public function reauthorize()
    {
        ...
        if (!$order->hasInvoices() && $authorizedAmount < $newOrderedAmount) {
            ...
            $this->savePayment();
            $paymentAuthorizenet->fetchTransactionsInfo($this->payment);
            return true;
        }
    ...
    private function updatePaymentTotals($order)
    {
        ...
        $this->payment->setAmountOrdered($newOrderedAmount);
        $this->payment->setBaseAmountOrdered($newBaseOrderedAmount);
        $this->payment->setShippingAmount($order->getShippingAmount());
        $this->payment->setBaseShippingAmount($order->getBaseShippingAmount());
    }
