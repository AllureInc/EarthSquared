...
class Shipping extends AbstractModel
{
    ...
    protected  function syncItemQtys(){
        ...
        foreach ($orderItems as $orderItem) {
            ...
            $productOrderItem = clone $this->product->create();
            $sku = $productOrderItem->load($orderItem->getProductId())->getSku();

            if($sku != $orderItem->getSku()){
                $orderItem->setSku($sku)->save();
            }
        }

        /** @var \Magento\Quote\Model\Quote\Item $quoteItem */
        foreach ($quoteItems as $quoteItem) {
            $productQuoteItem = clone $this->product->create();
            $sku = $productQuoteItem->load($quoteItem->getProductId())->getSku();

            $quoteItemId = $quoteItem->getItemId();
            if($sku != $quoteItem->getSku()){
