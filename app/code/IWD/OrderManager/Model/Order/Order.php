...
class Order extends \Magento\Sales\Model\Order
{
    ...
    protected function syncQuoteItems()
    {
        ...
        try {
            ...
            foreach ($this->getItems() as $orderItem) {
                $productSku = $this->productRepository->getById($orderItem->getProductId(), false, $storeId, true)->getSku();
                if($productSku != $orderItem->getSku()){
    ...
    private function getOptionsByItemId($itemId){

        $quote = $this->getQuote();
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();

        $quoteItem = $quote->getItemById($itemId);
        $quoteObj = $objectManager->create('Magento\Quote\Model\Quote\Item\Option');
        $options = $quoteObj->getCollection()->addItemFilter($itemId)->getOptionsByItem($quoteItem);
    ...
    private function restoreQuoteItem($orderItem)
    {
        if ($orderItem->getParentItem() === null) {
            ...
            if($options = $this->getOptionsByItemId($orderItem->getQuoteItemId())){
            ...
            $info->setProduct($orderItem->getProductId());
