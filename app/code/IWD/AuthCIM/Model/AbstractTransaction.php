...
class AbstractTransaction
{
    ...
    public function apiRequestMultishipping($request)
    {
        $response = [];
        $transferBuilder = $this->getTransferBuilder($request);
        $response = $this->authorizeNetRequest->placeRequest($transferBuilder);
        ...
        if ($this->getResponseParser()->isError($response)) {
            ...
            if ($this->getResponseParser()->getErrorCode($response) == 'E00015') {

                $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
                $cards = $objectManager->create('\IWD\AuthCIM\Model\Customer\Cards');
                $customerPaymentProfileId =  $cards->getCustomerPaymentProfilesFromAuthorizeNet($request['customerProfileId']);

                $response = [];
                $response["customerPaymentProfileId"] = $customerPaymentProfileId[0]['customerPaymentProfileId'];

            }elseif($this->getResponseParser()->getErrorCode($response) == 'E00039') {
                preg_match('!\d+!', $response['messages']['message']['text'], $matches);
                $response["customerProfileId"] = isset($matches[0]) ? $matches[0] : null;
            }else{
                throw new LocalizedException(__($this->getResponseParser()->getErrorMessage($response)));
            }
        }
