...
class Messages extends \Magento\Framework\View\Element\Template
{
    ...
    private function generateFullDiscount(Pack $pack): string
    {
        ...
        if ($pack->getDiscountType() == DiscountType::FIXED) {
            // initialize with 1 because need consider parent product
            $qtyProductsWithDiscount = 1;
            foreach ($this->getPackProductIds($pack) as $productId) {
                $qtyProductsWithDiscount += $pack->getChildProductQty($productId);
            }
            $result = $this->priceCurrency->format($qtyProductsWithDiscount * $pack->getDiscountAmount());
        }
