...
class ApiClient extends AbstractHelper
{
    ...
    /**
     * @var \Yotpo\Yotpo\Model\AbstractApi|null
     */
    ...
    public function __construct(
        ...
        if ($this->_moduleManager->isEnabled('Yotpo_Yotpo')) {
            $this->yotpoClient = $objectManager->get('Yotpo\Yotpo\Model\AbstractApi');
            $this->yotpoConfig = $objectManager->get('Yotpo\Yotpo\Model\Config');
        }
    ...
    private function updatePath($path, $storeId, $getParams = [])
    {
        $path = str_replace(
            self::APP_KEY,
            $this->yotpoConfig->getAppKey($storeId, 'stores'),
            $path
        );
